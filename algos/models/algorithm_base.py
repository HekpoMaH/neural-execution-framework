...
class AlgorithmBase(nn.Module):
    ...
    def loop_body(self,
        ...
        #     print('l', continue_logits)
        #     print('t', true_termination)
        #     from pprint import pprint
        #     pprint(list(zip(continue_logits.tolist(), true_termination.tolist())))
        #     input()
    ...
    def process(
        ...
        while True:
            ...
            if not self.loop_condition(
                    batch.termination[self.steps] if self.training or fit_decision_tree
                    or not self.concepts_frozen() else self.mask_cp,
                    STEPS_SIZE):
                break
